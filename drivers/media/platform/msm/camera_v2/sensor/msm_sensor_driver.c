	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	if (slave_info->camera_id == CAMERA_1) {
		if (slave_info->sensor_name &&
				!strncmp(slave_info->sensor_name, "imx179", strlen("imx179"))) { //Only imx179 need to match with PCB
...
CSID_TG:
	...
	*  sensor control structure */
	...
	if (!strncmp(slave_info->sensor_name, "imx179_4lane", 32)
			|| !strncmp(slave_info->sensor_name, "imx179", 32)) {
