...
#ifdef CONFIG_NUBIA_LCD_DISP_PREFERENCE
	pinfo->disable_dimming_when_suspend = of_property_read_bool(np,
		"nubia,disable-dimming-when-suspend");
	printk(KERN_INFO "[lcd] %s: %d: disable_dimming_when_suspend is %d\n",
		__func__, __LINE__, pinfo->disable_dimming_when_suspend);

	pinfo->enable_dimming_when_resume = of_property_read_bool(np,
		"nubia,enable-dimming-when-resume");
